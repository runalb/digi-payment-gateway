...
import com.runalb.ondemand_service.catalog.service.CatalogSuperAdminService;
...
@RestController
@RequestMapping("/api/v1/super-admin/catalog")
public class CatalogSuperAdminController {

    private final CatalogSuperAdminService catalogSuperAdminService;

    public CatalogSuperAdminController(CatalogSuperAdminService catalogSuperAdminService) {
        this.catalogSuperAdminService = catalogSuperAdminService;
    }

    @GetMapping("/categories")
    public ResponseEntity<List<CatalogCategoryResponse>> listCategories() {
        return ResponseEntity.ok(catalogSuperAdminService.listCategories());
    }

    @GetMapping("/categories/{categoryId}")
    public ResponseEntity<CatalogCategoryResponse> getCategory(@PathVariable Long categoryId) {
        return ResponseEntity.ok(catalogSuperAdminService.getCategory(categoryId));
    }

    @PostMapping("/categories")
    public ResponseEntity<CatalogCategoryResponse> createCategory(
            @Valid @RequestBody CatalogCategoryCreateRequest request) {
        CatalogCategoryResponse body = catalogSuperAdminService.createCategory(request);
    ...
    public ResponseEntity<CatalogCategoryResponse> updateCategory(
        ...
        return ResponseEntity.ok(catalogSuperAdminService.updateCategory(categoryId, request));
    }

    @DeleteMapping("/categories/{categoryId}")
    public ResponseEntity<Void> deleteCategory(@PathVariable Long categoryId) {
        catalogSuperAdminService.deleteCategory(categoryId);
    ...
    public ResponseEntity<List<CatalogServiceResponse>> listServices(@PathVariable Long categoryId) {
        return ResponseEntity.ok(catalogSuperAdminService.listServicesInCategory(categoryId));
    }
    ...
    public ResponseEntity<CatalogServiceResponse> createService(
        ...
        CatalogServiceResponse body = catalogSuperAdminService.createService(categoryId, request);
        return new ResponseEntity<>(body, HttpStatus.CREATED);
    }

    @GetMapping("/services/{serviceId}")
    public ResponseEntity<CatalogServiceResponse> getService(@PathVariable Long serviceId) {
        return ResponseEntity.ok(catalogSuperAdminService.getCatalogService(serviceId));
    }
    ...
    public ResponseEntity<CatalogServiceResponse> updateService(
        ...
        return ResponseEntity.ok(catalogSuperAdminService.updateCatalogService(serviceId, request));
    }

    @DeleteMapping("/services/{serviceId}")
    public ResponseEntity<Void> deleteService(@PathVariable Long serviceId) {
        catalogSuperAdminService.deleteCatalogService(serviceId);
        return ResponseEntity.noContent().build();
    }
}
